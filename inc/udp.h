...
struct UdpSocket {
  ...
  void close_socket() {
    if (handle != -1) {
      ...
      //::close(handle);
 	// there is no close in the class.
	::close(handle);
#endif
      handle = -1;
    }
  ...
  std::string localHostName() const {
    ...
    if (gethostbyname(hostname_buf) != 0)
      hostname_buf[0] = 0;
  ...
  */
  ...
  SockAddr Sub_packetOrigin() { return remote_addr; }
...
private:
  ...
  bool openSocket(const std::string &hostname, const std::string &port, int options) {
    ...
    close_socket(); error_message.clear();
